...
*   [x] **Phase 1: Discovery and Analysis** (Completed)
    ...
    *   [x] Compare and document pattern implementations.
...
*   [ ] **Phase 3: Refactor Strategies** (In Progress)
    *   [x] Refactor `breakout_trading_strategy.py`.
        ...
        *   **Note**: Morning/Evening Star complex versions were replaced by standard TA-Lib versions. Strong reversal candle is still expected from `cp`.
        *   Updated `generate_signals` and `_detect_retest` to use centralized functions and TA-Lib.
    *   [x] Refactor `breakout_reversal_strategy.py`.
        *   Removed `cp` import and usages.
        *   Integrated `talib` for Hammer, Shooting Star, Engulfing, Morning/Evening Star patterns.
        *   Removed `detect_key_reversal_bar_after_breakout` checks as it was specific to `cp`.
    *   [x] Refactor `trend_following_strategy.py`.
        *   **Linter Errors**: Type errors when passing pandas Series values to TA-Lib functions. Resolved by casting to `np.array(..., dtype=np.float64)`.
        *   **TODO**: Ensure TA-Lib is correctly installed in the virtual environment and added to `requirements.txt`.
        *   Used `_detect_inside_bar_vectorized` static method for inside bars as TA-Lib lacks a direct equivalent.
    *   [x] Refactor `price_action_sr_strategy.py`.
        *   Removed `cp` import and usages.
        *   Integrated `talib` for Hammer, Shooting Star, Engulfing, Morning/Evening Star, Doji, Harami patterns.
        *   Defined TA-Lib patterns (e.g. `hammer_talib`) in `generate_signals` and passed boolean series to `_pattern_match`.
    *   [x] Refactor `confluence_price_action_strategy.py`.
        *   Replaced `cp` calls in `_is_pin_bar`, `_is_engulfing`, `_is_hammer`, `_is_shooting_star`, `_is_morning_star`, `_is_evening_star` with `talib` equivalents.
        *   Updated `_is_inside_bar` to use the class's own `_detect_inside_bar_vectorized` method.
        *   Ensured TA-Lib functions receive NumPy arrays.
*   [ ] **Phase 4: Finalization**
    *   [ ] Review and finalize documentation for the new module and strategies.
    *   [ ] Update scratchpad with completion status and lessons.

## Executor's Feedback or Assistance Requests

- **DONE:** Addressed `Expression value is unused` for `retest_price_info` in `breakout_trading_strategy.py`.
- **NEW:** User is experiencing an issue where a package installed via an EXE is not recognized in the project's virtual environment, causing errors in Cursor. Need to identify the package, the venv details, and the exact error message to guide the user on how to make the package available to the venv. 
    - **Guidance provided:** Steps to install TA-Lib into the `.venv` using pip (preferably with a wheel file on Windows) and update `requirements.txt`.
- **LINTER ERROR TO ADDRESS**: `breakout_trading_strategy.py` is referencing `detect_strong_reversal_candle` from `cp` which do not exist yet in `src/utils/candlestick_patterns.py`. These need to be implemented based on the logic previously in `breakout_trading_strategy.py`.

## Lessons

- When a variable is reported as unused, and its value is primarily for debugging or logging, consider inlining its value directly into the log message and removing the variable assignment to clean up the code.
- Packages installed globally (e.g., via an EXE installer) are generally not automatically available within a Python virtual environment. The package needs to be installed *into* the venv, or the venv needs to be configured to find it.
- TA-Lib functions expect NumPy arrays of specific dtypes (e.g., `np.float64`). Explicitly casting pandas Series values (e.g., `df['column'].values`) to `np.array(..., dtype=np.float64)` can resolve type errors with linters.